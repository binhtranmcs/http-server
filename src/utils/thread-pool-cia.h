...
public:
  ...
  thread_local_queue_thread_pool(unsigned int const num_threads) : done(false), joiner(threads) {
    try {
      for (unsigned int i = 0; i < num_threads; ++i) {
        threads.emplace_back(&thread_local_queue_thread_pool::worker_thread, this);
      }
    } catch (...) {
      done = true;
      throw;
...
public:
  ...
  work_stealing_thread_pool(unsigned int thread_count) : done(false), joiner(threads) {
    try {
      for (unsigned int i = 0; i < thread_count; ++i) {
        queues.push_back(std::make_unique<work_stealing_queue>());
      }
      for (unsigned int i = 0; i < thread_count; ++i) {
        threads.emplace_back(&work_stealing_thread_pool::worker_thread, this, i);
      }
    } catch (...) {
      done = true;
      throw;
